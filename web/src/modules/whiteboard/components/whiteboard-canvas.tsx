...
import { requestWhiteboardWsTicketAction } from "@/actions/ws-tickets"
import {
  connectRoom,
  disconnectRoom,
  addMessageListener,
...
export function WhiteboardCanvas({ boardId, workspaceId, slug }: WhiteboardCanvasProps) {
  ...
  // Capture boardId/workspaceId in refs so the store initializer (which runs
                    ...
                    if (res.success && res.base64 && res.mimeType) {
                      ...
                      if (editorRef.current) {
                        editorRef.current.updateAssets([
                          { ...asset, meta: { ...asset.meta, resolvedAt: Date.now() } },
                        ])
                      }
                    }
                  })
                  .catch((err) => console.error("Failed to load asset via server action:", err))
              }
            }
          }
          return src ?? null
  ...
  // Connect to the WS room on mount. The ticket is fetched via a server action
  // so the access token never touches client JS.
  useEffect(() => {
    connectRoom({
      roomId: boardId,
      wsUrl: process.env.NEXT_PUBLIC_WHITEBOARD_WS_URL ?? "ws://localhost:9093",
      path: `/ws/boards/${boardId}`,
      fetchTicket: () => requestWhiteboardWsTicketAction(),
    }).catch(() => {})
  ...
  useEffect(() => {
    ...
    const sendOp = async () => {
      ...
      sendRoomMessage(
        boardId,
        JSON.stringify({ type: "snapshot", clock: Date.now(), document: snapshot.document }),
      )
    }
